...
try {
    ...
    error_log("Database error in get_businesses: " . $e->getMessage());
    error_log("Stack trace: " . $e->getTraceAsString());
    http_response_code(500);
    $errorMessage = 'Failed to fetch businesses. Please try again later.';
    if (ini_get('display_errors')) {
        $errorMessage .= ' Error: ' . $e->getMessage();
    }
    echo json_encode(['success' => false, 'message' => $errorMessage]);
} catch(Exception $e) {
    error_log("General error in get_businesses: " . $e->getMessage());
    error_log("Stack trace: " . $e->getTraceAsString());
    http_response_code(500);
    $errorMessage = 'Failed to fetch businesses. Please try again later.';
    if (ini_get('display_errors')) {
